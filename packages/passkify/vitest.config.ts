 ...
 * surface, and it lets one flag point the same tests at the compiled output
 * instead:
 *
...
const aliasesFor = (target: 'src' | 'dist') =>
  target === 'dist'
    ? {
...
/**
 * `--mode dist` rather than an environment variable.
 *
 * The script used to set `PASSKIFY_TEST_TARGET=dist` inline, which cmd.exe does
 * not understand — the Windows row failed with "'PASSKIFY_TEST_TARGET' is not
 * recognized". vitest already has a mode flag and it works the same on every
 * shell. The variable is still honoured so an existing invocation keeps working.
 */
export default defineConfig(({ mode }) => {
  const target: 'src' | 'dist' =
    mode === 'dist' || process.env.PASSKIFY_TEST_TARGET === 'dist' ? 'dist' : 'src';
